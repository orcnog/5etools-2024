	...
	this._recursiveRender = function (entry, textStack, meta, options) {
		...
		if (typeof entry === "object") {
			...
			switch (type) {
				...
				case "encounter": globalThis.RendererEncounterBlock?.render?.call(this, entry, textStack, meta, options); break;
	...
	this._renderString_renderTag = function (textStack, meta, options, tag, text) {
		...
		switch (tag) {
			...
			case "@note":
				...
				break;
			case "@cue":
				if (globalThis.RendererMediaCues) globalThis.RendererMediaCues.handleCueTag(this, {text, textStack, meta});
				break;
			case "@tip": {
	...
	static _bindListenersScale (mon, ele) {
			...
			?.onn("click", evt => {
				...
				Renderer.monster.getCrScaleTarget({
					...
					cbRender: async (targetCr) => {
						...
						if (globalThis.jQuery) globalThis.jQuery(ele).trigger("cr_update", [toRender]);
		...
		ele
			...
			?.onn("click", async () => {
				...
				ele.empty().appends(fnRender(toRender));
				if (globalThis.jQuery) globalThis.jQuery(ele).trigger("cr_reset", [toRender]);
